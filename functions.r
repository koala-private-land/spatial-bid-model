get.jags.sel <- function(Data)
{
	...
	inits3 <- list(WTA = get.WTA.inits(Data$WTA, Data$CENS), sig = runif(1, 0, 2), phi = runif(1, 0, 2), b_x = runif(Data$NCOV, -2, 2), b_y = runif(Data$NCOV, -2, 2), b_z = runif(Data$NCOV, -2, 2), bsa_x = c(NA, runif(Data$NCOVSA - 1, -2, 2)), bsa_y = c(NA, runif(Data$NCOVSA - 1, -2, 2)), bsa_z = c(NA, runif(Data$NCOVSA - 1, -2, 2)))
  	...
  	fit <- run.jags(model="wta-jags-model-sel.txt", monitor = c("eta_g_x", "eta_g_y", "eta_g_z", "gammac_gl_x", "gammac_gl_y", "gammac_gl_z",
		...
		jags = "C:/Program Files/JAGS/JAGS-4.3.1/x64/bin/jags-terminal.exe", method = "rjparallel", cl = cl) # method = "rjags")
...
get.jags.postcheck <- function(Data)
{
  	# get initial values for censored WTA
  	get.WTA.inits <- function(WTA, CENS) {
      Inits.WTA <- rep(NA, length(WTA))
	...
	inits3 <- list(WTA = get.WTA.inits(Data$WTA, Data$CENS), sig = runif(1, 0, 2), phi = runif(1, 0, 2), b_x = runif(Data$NCOV, -2, 2), b_y = runif(Data$NCOV, -2, 2), b_z = runif(Data$NCOV, -2, 2), bsa_x = c(NA, runif(Data$NCOVSA - 1, -2, 2)), bsa_y = c(NA, runif(Data$NCOVSA - 1, -2, 2)), bsa_z = c(NA, runif(Data$NCOVSA - 1, -2, 2)))
  	...
  	# edit methods depending on whether using parallel processing or not
  	fit <- run.jags(model="wta-jags-model-postcheck.txt", monitor = c("fit_accept_test", "fit_wta_test", "fit_prop_test"), data = Data, n.chains = 3, inits = list(inits1,inits2,inits3), burnin = 10000, adapt = 1000, sample = 10000, jags = "C:/Program Files/JAGS/JAGS-4.3.1/x64/bin/jags-terminal.exe", method = "rjparallel", cl = cl) # method = "rjags")
