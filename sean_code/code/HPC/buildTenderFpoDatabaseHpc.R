...
getScriptPath <- function() {
  ...
  if (length(fileArgument) == 0) {
    return(file.path(getwd(), "buildTenderFpoDatabaseHpc.R"))
  }
...
readWorkbookMetadata <- function(workbookPath) {
  workbookTempDirectory <- tempfile("xlsx_", tmpdir = scratchRoot)
...
writeAttributeLayer <- function(attributeData, outputPath, layerName) {
  tempCsvPath <- tempfile(fileext = ".csv", tmpdir = scratchRoot)
...
projectPath <- normalizePath(dirname(scriptPath), winslash = "/", mustWork = TRUE)
inputPath <- normalizePath(
  Sys.getenv("TENDER_FPO_INPUT_ROOT", unset = file.path(projectPath, "inputs")),
  winslash = "/",
  mustWork = TRUE
)
outputRoot <- normalizePath(
  Sys.getenv("TENDER_FPO_OUTPUT_ROOT", unset = file.path(projectPath, "outputs")),
  winslash = "/",
  mustWork = FALSE
)
scratchRoot <- normalizePath(
  Sys.getenv("TENDER_FPO_SCRATCH_DIR", unset = tempdir()),
  winslash = "/",
  mustWork = FALSE
)
...
if (!dir.exists(scratchRoot)) {
  dir.create(scratchRoot, recursive = TRUE)
}

workerOverride <- suppressWarnings(as.integer(Sys.getenv("TENDER_FPO_WORKERS", unset = NA_character_)))
outputPath <- file.path(outputRoot, "tenderFpoDatabase.gpkg")
...
message("Project root: ", projectPath)
message("Input root: ", inputPath)
message("Output root: ", outputRoot)
message("Scratch root: ", scratchRoot)
...
propertyLgaTime <- system.time({
  propertyWorkerPath <- file.path(scratchRoot, "availableProperties.gpkg")
  lgaWorkerPath <- file.path(scratchRoot, "lgaBoundaries.gpkg")
  propertyLgaChunkDirectory <- file.path(scratchRoot, "property_lga_chunks")
  ...
  if (!is.na(workerOverride) && workerOverride > 0) {
    propertyLgaWorkerCount <- min(propertyLgaWorkerCount, workerOverride)
  }
...
zeroPropertyLgas <- fixedPriceRegions %>%
  anti_join(availableLgas, by = "LGA") %>%
  distinct(LGA) %>%
  arrange(LGA) %>%
  pull(LGA)

message("")
message("Tender-fpo database HPC run complete")
